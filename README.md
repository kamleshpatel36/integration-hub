...
## About the initial migration

`prisma/migrations/20260101000000_init/migration.sql` was hand-written
directly against `schema.prisma` rather than generated by
`npx prisma migrate dev`, because generating it requires downloading engine
binaries from `binaries.prisma.sh`, which wasn't reachable from the
environment this project was built in. It was verified by applying it
directly to a real local Postgres 16 instance with `psql` — every table,
enum, index, and foreign key was checked to match the schema exactly.

This is a one-time thing, not an ongoing pattern. Every migration **after**
this one should be generated normally:
```
npx prisma migrate dev --name whatever_you_changed
```
from a machine with a working Postgres connection and normal internet
access — that's the standard Prisma workflow, and it'll work fine outside
